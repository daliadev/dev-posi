		...
		*/
		...
		{	
			if (strlen($categorie->getCode()) == 2 && $categorie->getHasResult())
			{
				...
				$messageBody .= '<br>';
				$messageBody .= '<span style="font-size: 10px;">'.$categorie->getNom().' / <strong>'.$categorie->getScorePercent().'</strong>% ('.$categorie->getTotalReponsesCorrectes().'/'.$categorie->getTotalReponses().' questions)</span>';
			}
		}
		$messageBody .= '</p>';
		$messageBody .= '<br>';
